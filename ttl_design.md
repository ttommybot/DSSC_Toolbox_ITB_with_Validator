...
这里需要区分 **TTL 规则** 和 **完整 Test Case 规则**：

- 本 TTL 本身不会因为缺少 `license` 而判定失败；
- 但 TC02 另外包含独立的 License 测试，而且 TC02 不使用这份 TTL；
- 因此，在 TC02 中，缺少 `license` 的输入仍然会被判定为不通过。
...
本 TTL：不因该字段缺失而失败
    ↓
TC02 的独立 License 检查：FAIL
```

因此，“TTL 中 license 可选”与“TC02 要求 license 必须存在”并不冲突，两者属于不同层次的验证规则。
...
这些内容超出当前 SHACL Shapes 的设计范围，因此不属于本 `.ttl` 文件的验证职责。
...
本 TTL 在现有 Building Energy Metadata 结构上定义了一套 SHACL conformance rules，主要覆盖：

- Dataset 必须唯一且具有 IRI；
- 必填字段必须存在且保持单值；
- 必填字符串不能是空字符串或纯空白；
- `format`、`frequency`、`unit` 必须属于允许值集合；
- `temporalStart` 和 `temporalEnd` 必须为合法日期，并满足开始时间不晚于结束时间；
- `endpointUrl` 必须唯一、为 IRI 且使用 HTTPS；
- `license` 在当前 TTL 中为可选字段，但如果提供，必须唯一、为 IRI 且使用 HTTPS；
- `description` 为可选单值字符串；
- Dataset 不允许出现 profile 未声明的额外属性。

因此，该 `.ttl` 文件的职责是定义 **Building Energy Dataset Metadata 的结构和语义约束**，并由 SHACL Validator 根据这些规则判断输入 RDF/JSON-LD metadata 是否符合该 profile。
