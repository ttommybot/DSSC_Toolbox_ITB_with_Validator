...
只有 `pass` 可以通过 onboarding；`fail`、`inapplicable`、`untestable` 都应被拒绝。当前官方 validator 可以覆盖基础 SHACL / JSON Schema 校验；如果要完整实现四状态业务映射，尤其是 `inapplicable` 和 license policy，需要后续 validator wrapper 或 custom validation service。
